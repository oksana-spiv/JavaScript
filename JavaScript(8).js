...
document.addEventListener('click', function(event) {
    const elemFirst = event.target.classList.contains('super_element');
    let elemSecond;
    if (elemFirst) {
        elemSecond = 'присутствует';
    } else {
        elemSecond = 'остутствует';
    }
    console.log(`Класс "super_element" ${elemSecond} в элементе "${event.target.tagName.toLowerCase()}".`);
    });
...
// 5. Необходимо повесить событие клика на тег ul. В обработчике события в консоль необходимо выводить текст, который записан внутри элемента кнопки, по которой был произведен клик. 
...
// Это связано с порядком выполнения событий.  Сначала отрабатывает событие по тегу ul, а потом по родительскому элементу - document, которое происходит при клике на любой элемент страницы. 

 

// 7. С помощью JS необходимо изменить цвет заднего фона каждого второго тега li.
document.querySelectorAll('li:nth-child(2n)')
 .forEach(el => el.style.backgroundColor = 'braun');
